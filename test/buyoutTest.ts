...
describe('NibblTokenVault', function () {
    ...
    it("Buyout succeeds when time passes and twav<buyoutrejectionvaluation throughout the 3 days", async function () {
        ...
        await this.tokenVault.connect(this.buyer1).initiateBuyOut({value:buyoutBid});
        await network.provider.send("evm_increaseTime", [3*24*60*60])
        //TODO add unlock methods
        await expect(this.tokenVault.connect(this.buyer1).buy(0, this.buyer1.address, { value: _buyAmount })).to.revertedWith('NFT has been bought')
    })
    ...
    it(" Mint/Burn stops after success", async function () {
        ...
        await network.provider.send("evm_increaseTime", [3*24*60*60])
        await expect(this.tokenVault.connect(this.buyer1).buy(0, this.buyer1.address, { value: _buyAmount })).to.revertedWith('NFT has been bought')
        //TODO write test case for sell
        // await expect(this.tokenVault.connect(this.buyer1).sell(0, this.buyer1.address, { value: _buyAmount })).to.revertedWith('NFT has been bought')
    })
    it("No more buyout bids possible", async function () {
        const _buyAmount = ethers.utils.parseEther("1");
        const buyoutBid = ethers.utils.parseEther("200");
        await this.tokenVault.connect(this.buyer1).initiateBuyOut({value:buyoutBid});
        await expect(this.tokenVault.connect(this.addr1).initiateBuyOut({value:buyoutBid})).to.revertedWith('NibblVault: Only when initialised');
    })
    it("Buyout rejects automatically when twav>=buyoutrejectionvaluation within 3 days", async function () {
        const _buyAmount = ethers.utils.parseEther("1");
        const buyoutBid = ethers.utils.parseEther("90");
        await this.tokenVault.connect(this.buyer1).initiateBuyOut({value:buyoutBid});
        const balanceBeforeRejection = await this.admin.provider.getBalance(this.buyer1.address)
        for(let i = 0 ; i < 12 ; i++){
            this.tokenVault.connect(this.addr1).buy(0, this.buyer1.address, { value: _buyAmount })
        }
        const balanceAfterRejection = await this.admin.provider.getBalance(this.buyer1.address)
        console.log(balanceBeforeRejection,balanceAfterRejection)
    })
    it("Buyout bid is rejected if the valuation is low", async function () {
        const buyoutBid = ethers.utils.parseEther("1");
        await expect(this.tokenVault.connect(this.buyer1).initiateBuyOut({value:buyoutBid})).to.revertedWith('NibblVault: Low buyout valuation');
    })

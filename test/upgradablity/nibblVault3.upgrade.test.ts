...
import { Basket, Basket__factory, ERC1155Link, ERC1155Link__factory, ERC721TestToken, ERC721TestToken__factory, NibblVault, NibblVault2, NibblVault2__factory, NibblVault3, NibblVault3__factory, NibblVaultFactory, NibblVaultFactory__factory, NibblVault__factory, TestBancorFormula, TestBancorFormula__factory } from "../../typechain-types";
import * as constants from "../constants";
import { getBigNumber, getCurrentValuation } from "../helper";
import { TWAV } from "../twav";

describe("Upgrade: NibblVault3", function () {
    ...
    async function deployNibblVaultFactoryFixture() {
        ...
        //Deploy NibblVault2Implementation
        ...
        //Deploy NibblVault2Implementation
        const NibblVault3_Factory: NibblVault3__factory = await ethers.getContractFactory("NibblVault3");
        const vaultImplementation3: NibblVault3 = await (await NibblVault3_Factory.deploy(erc1155LinkImplementation.address)).deployed();
        ...
        //create a vault
        await vaultFactoryContract.connect(curator).createVault(erc721Token.address, curator.address, constants.tokenName, constants.tokenSymbol, 0, constants.initialTokenSupply, constants.initialTokenPrice, (await time.latest()) + time.duration.days(6), { value: constants.initialSecondaryReserveBalance });
        ...
        const vaultContract: NibblVault3 = NibblVault3_Factory.attach(proxyAddress)
        // await vaultContract.connect(user1).buy(0, user1.address, { value: ethers.utils.parseEther("1") })

        await vaultFactoryContract.connect(implementationRole).proposeNewVaultImplementation(vaultImplementation3.address);
        await time.increase(constants.UPDATE_TIME_FACTORY);
    ...
    describe("Trade", function () {
        it("should buy tokens successfully from primary curve", async function () {
            ...
            const _initialSupply = await vaultContract.totalSupply()
            const _initialFeeAccruedCurator = await vaultContract.feeAccruedCurator()
            //
            const _buyAmountWithFee = _buyAmount.sub(_buyAmount.mul(constants.FEE_TOTAL).div(constants.SCALE));

            const _purchaseReturn = await testBancorFormulaContract.calculatePurchaseReturn(_initialSupply, _initialPrimaryBalance, constants.primaryReserveRatio, _buyAmountWithFee);
            ...
            expect((await vaultContract.feeAccruedCurator()).sub(_initialFeeAccruedCurator)).to.equal((_buyAmount.mul(constants.FEE_CURATOR)).div(constants.SCALE));
            if (admin.provider) {
      ...
      describe("ERC1155 Link", () => {
        it("should create a ERC1155Link", async function () {
          const { vaultContract, user1, curator } = await loadFixture(deployNibblVaultFactoryFixture);

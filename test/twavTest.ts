...
//     const _initialSecondaryBalance =
//       await this.tokenVault.secondaryReserveBalance();
//     const _initialPrimaryBalance =
//       await this.tokenVault.primaryReserveBalance();
//     const _buyAmountWithFee = _buyAmount.sub(
//       _buyAmount.mul(_feeTotal).div(SCALE)
//     );
//     const _purchaseReturn = await mintTokens(
//       this.testBancorBondingCurve,
//       initialTokenSupply,
//       primaryReserveBalance,
//       primaryReserveRatio,
//       _buyAmountWithFee
//     );
//     const _newSecBalance = _initialSecondaryBalance.add(
//       _buyAmount.mul(FEE_CURVE).div(SCALE)
//     );
//     const _newSecRatio = _newSecBalance.mul(SCALE).div(initialValuation);
//     const _newPrimaryReserveBalance =
//       _initialPrimaryBalance.add(_buyAmountWithFee);
//     await this.tokenVault
//       .connect(this.buyer1)
//       .buy(_purchaseReturn, this.buyer1.address, { value: _buyAmount });
//     expect(await this.tokenVault.secondaryReserveBalance()).to.equal(
//       _newSecBalance
//     );
//     expect(await this.tokenVault.primaryReserveBalance()).to.equal(
//       _newPrimaryReserveBalance
//     );
//     expect(await this.tokenVault.secondaryReserveRatio()).to.equal(
//       _newSecRatio
//     );
//     const _observation = await this.tokenVault.twavObservations(
//       BigNumber.from(await this.tokenVault.twavObservationsIndex()).sub(ONE)
//     );
//     expect(_observation.cumulativeValuation).to.equal(
//       _observation.timestamp.mul(initialValuation)
//     );
//     await network.provider.send("evm_increaseTime", [3600]);
//     const _valuation: BigNumber = _newSecBalance
//       .mul(SCALE)
//       .div(_newSecRatio)
//       .add(
//         _newPrimaryReserveBalance
//           .sub(fictitiousPrimaryReserveBalance)
//           .mul(SCALE)
//           .div(primaryReserveRatio)
//       );
...
//     // }
//     const _observation2 = await this.tokenVault.twavObservations(
//       BigNumber.from(await this.tokenVault.twavObservationsIndex()).sub(ONE)
//     );
//     expect(_observation2.cumulativeValuation).to.equal(
//       _observation.cumulativeValuation.add(
//         _valuation.mul(_observation2.timestamp - _observation.timestamp)
//       )
//     );
//   });
...
//       await this.testTWAV._updateTWAV(
//         initialValuationArray[i],
//         fakeTimestamp.toFixed(0)
//       );
